	...
	if (Game::gt.timeSince(created) > limit) {
		events.push(
			TargettedEvent::UPtr(new TargettedEvent(
				TimerEvent::create(self()),               // Emit the timer event with self() as the source
				listener                                  // To the listener
			))
		);
		events.push(RemoveControllerEvent::create(this)); // Then remove/delete yourself (the timer)
	}
}
